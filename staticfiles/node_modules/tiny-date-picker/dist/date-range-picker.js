(function (global, factory) {
  ...
  function defaults() {
    return {
      ...
      // Possible values: dp-modals, dp-below, dp-permanent
      mode: 'dp-modals',
  ...
  * @param {DatePickerContext} dp
  ...
  function focusInput(input) {
    // When the modals closes, we need to focus the original input so the
  ...
  /**
   * @file Defines the modals date picker behavior.
   */

  function ModalMode(input, emit, opts) {
    var dp = BaseMode(input, emit, opts);

    // In modals mode, users really shouldn't be able to type in
    // the input, as all input is done via the calendar.
    input.readonly = true;

    // In modals mode, we need to know when the user has tabbed
  ...
  /**
   * @file Defines the various date picker modes (modals, dropdown, permanent)
   */

  function Mode(input, emit, opts) {
    input = input && input.tagName ? input : document.querySelector(input);

    if (opts.mode === 'dp-modals') {
  ...
  * @property {('dp-modals'|'dp-below'|'dp-permanent')} [mode] - The date picker mode, defaults to 'dp-modals'
